...
def f1_score(preds, true_values):
    preds = torch.where(preds>0.5, 1, 0)
    tp = torch.sum(preds * true_values)
    fp = torch.sum(preds * (1-true_values))
    fn = torch.sum((1-preds) * true_values)
    precision = tp / (tp + fp)
    recall = tp / (tp + fn)
...
f1 = f1_score(output.view(-1), test_set[:][:, -1])

print("Test set: loss: {} f1: {} acc: {}".format(loss.item(), f1, acc))
